...
handle = open("C:\Users\Αναστασία\Desktop\Daily_work\Raw_materials_for_samples\ordered_PDB.fasta","rU")
ordered = list(SeqIO.parse(handle,"fasta"))
handle.close()
...
c= open('C:\Users\Αναστασία\Desktop\Daily_work\Raw_materials_for_samples\surf_PDB.txt', 'r')
    ...
    if b[2]!=Name:
        ...
        for index in range (len(net_charge)):
            ...
            output1.write(Name +"_"+ str(Position[index])+ (" ")*space + '\t'+ Aminoacid[index]  +'\t'+ str(net_charge[index]) +'\t' +str(hydrophobicity[index]) +'\t' + str(cluster[index]) +'\t' + str(cluster2[index]) +'\t' + str(Buried_Exposed[index]) +'\t' + \
                          ...
                          str(RSA[index]) +'\t' + str(ASA[index]) +'\t' + str(Alpha_Helix[index]) +'\t' + str(Beta_Strand[index]) +'\t' +str(Coil[index]) +'\t' +str(0) + '\n')
